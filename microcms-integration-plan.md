...
| `order` | 数字 | — | ギャラリー表示順（小さいほど先）。管理画面の DnD 並び替えで更新 |

> フィールド ID はコードと一致させてください（`title` / `charge` / `image` / `order`）。  
> 既存サービスに `order` が無い場合は追加してください。未設定のコンテンツは `publishedAt` でフォールバックします。
